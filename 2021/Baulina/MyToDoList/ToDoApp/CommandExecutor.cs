...
namespace ToDoApp
{
    public class CommandExecutor : IExecute
    {
        ...
        public async Task<ToDoList> GetActualToDoList()
        {
            var response = await Call(() => _client.GetAsync(RequestUri));
            var responseString = await response.Content.ReadAsStringAsync();
            return new ToDoList(JsonConvert.DeserializeObject<IEnumerable<ToDoItem>>(responseString));
        }
        ...
        public int ChooseTaskNumber(ToDoList toDoList)
        {
            ...
            if (result >= 0 && result <= toDoList[^1].Id)
                return result;
            throw new ArgumentOutOfRangeException();
        }

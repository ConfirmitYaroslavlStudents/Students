...
namespace Hospital.Tests
{
    ...
    {
        private static readonly DatabaseProvider DatabaseProvider = new DatabaseProvider();
        private readonly AnalysisProvider _analysisProvider = new AnalysisProvider(DatabaseProvider);
        private readonly PersonProvider _personProvider = new PersonProvider(DatabaseProvider);
        private readonly TemplateProvider _templateProvider = new TemplateProvider(DatabaseProvider);
        ...
        public void PersonProvider_SaveAndSearch_ShouldPass(string firstName, string lastName, string middleName,
            string birthDate)
        {
            var person = new Person(firstName, lastName, middleName, DateTime.Parse(birthDate));
            _personProvider.Save(ref person);
            IList<Person> loadedPerson = _personProvider.Search(firstName, lastName);
        ...
        public void HtmlToTextParser_HtmlTemplateIsProperlyParsed_ShouldPass(string pathToHtmlTemplate,
            string pathToTxtTemplate)
        {
            var htmlParser = new HtmlToTextParser();
            string htmlTemplate = GetTemplate(pathToHtmlTemplate);
            string actualTextTemplate = htmlParser.Parse(htmlTemplate).Replace("\r\n", "").Replace("\n", "");
            string expectedTextTemplate = GetTemplate(pathToTxtTemplate).Replace("\r\n", "").Replace("\n", "");
        ...
        public void TemplateProvider_PersonAnalyzesArePropelyLoaded_ShouldPass(string name, string content)
        {
            ...
            IList<Template> templates = _templateProvider.Load();
            _templateProvider.Remove(expectedTemplate);

            Template actualTemplate = templates[0];

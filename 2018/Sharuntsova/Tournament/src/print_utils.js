...
function buildColumns(typeOfDisplay, bracket, numberOfRounds, playerNameLength) {
    let printingColumns = [];
    switch (typeOfDisplay) {
        case TypesOfPrinting.Olympics:
            buildOlympicsBracket(printingColumns, bracket, numberOfRounds, playerNameLength);
            break;
        case TypesOfPrinting.DoubleBracket:
...
function buildLeftPartColumns(printingColumns, bracket, numberOfRounds, playerNameLength) {
    ...
    for (let i = 0; i < numberOfRounds - 1; i++) {
        ...
        printingColumns.push(makeVerticalBrackets(bracket[i].length / 2, lineSpace, edgeSpace, cornerUpLeft, cornerDownLeft)); //bracket[i].length / 2
    }
}
...
function buildOlympicsBracket(printingColumns, bracket, numberOfRounds, playerNameLength) {
    ...
    for (let i = 0; i < numberOfRounds; i++) {
        ...
        printingColumns.push(currentRow);
        printingColumns.push(makeVerticalBrackets(bracket[i].length, lineSpace, edgeSpace, cornerUpLeft, cornerDownLeft)); //bracket[i].length / 2
    }

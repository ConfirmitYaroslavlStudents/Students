...
namespace Creaturestests.Food
{
    ...
    public class FillingOfEntireFieldStrategyTests
    {
        ...
        public void BuildTest()
        {
            ...
            for (int k = 0; k < 10; k++)
            {
                ...
                for (int i = 0; i < eatMatrix.Length; i++)
                    for (int j = 0; j < eatMatrix.Height; j++)
                        Assert.AreEqual(FoodMatrixConstants.FoodLevel, eatMatrix.GetLevelOfFood(new Point(i, j)));
            }
            ...
            for (int i = 0; i < eatMatrix.Length; i++)
                for (int j = 0; j < eatMatrix.Height; j++)
                    Assert.AreEqual(FoodMatrixConstants.FoodLevel * 2, eatMatrix.GetLevelOfFood(new Point(i, j)));
        }
    }
}